...
| <<nfs_verify_stats_csv,verifique -stats>> | Verifica as árvores de origem e destino em paralelo e compara estatísticas de árvore. 


| <<nfs_verify_stats_csv,verifique -csv>> | Verifica as árvores de origem e destino em paralelo e compara estatísticas de árvore. 


| <<nfs_verify_nodata,verifique -nodata>> | Não verifica os dados. 


| <<nfs_verify_noatts,verifique -noattrs>> | Não verifica atributos. 


| <<nfs_verify_nomods,verifique -nomods>> | Não verifica os tempos de modificação do ficheiro. 


| <<nfs_verify_mtimewindow,verificar -mtimewindow >> | Especifica a diferença de tempo de modificação aceitável para verificação. 


| <<nfs_verify_v_l,verifique -v>> | Recupera formatos de saída para listar quaisquer diferenças encontradas. 


| <<nfs_verify_v_l,verificar -l>> | Recupera formatos de saída para listar quaisquer diferenças encontradas. 


| <<nfs_verify_nonames,verifique -nomes diferentes>> | Exclui nomes de usuários e grupos de listas de arquivos ou relatórios. 


| <<nfs_verify_match,verificar -corresponder >> | Apenas processa arquivos e diretórios que correspondem ao formato. 


| <<nfs_verify_bs,verificar -bs [k]> | Especifica o tamanho do bloco de leitura/gravação (padrão: 64K). 


| <<nfs_verify_parallel,verificar -paralelo >> | Especifica o número máximo de processos em lote simultâneos (padrão: 7). 


| <<nfs_verify_dircount,verificar -dircount [k]> | Especifica o tamanho da solicitação ao ler diretórios. 


| <<nfs_verify_noid,Verifique -noid>> | Desativa a criação de um índice padrão (default: False). 


| <<nfs_verify_preserveatime,verifique -preserve-atime>> | Restaura todos os arquivos para a última data acessada na origem. 


| <<nfs_verify_s3_insecure,verifique o -s3.insecure>> | Fornece a opção de usar HTTP em vez de HTTPS para comunicação de bucket S3. 


| <<nfs_verify_s3_endpoint,verificar -s3.endpoint >> | Substitui o URL padrão do endpoint do Amazon Web Services (AWS) com o URL especificado para a comunicação de bucket do S3. 


| <<nfs_verify_s3_profile,verificar -s3.profile >> | Especifica um perfil do arquivo de credenciais da AWS para comunicação de bucket do S3. 


| <<nfs_verify_s3_noverify,verifique o -s3.noverify>> | Substitui a verificação padrão da certificação SSL para comunicação de bucket S3. 
|===


== verifique -stats e verifique -csv

Use os `-stats` parâmetros e `-csv` com o `verify` comando para verificar as árvores de origem e destino em paralelo e comparar estatísticas de árvore.
...
Use o `-nodata` parâmetro com o `verify` comando para especificar não verificar dados.
...
Use o `-noattrs` parâmetro com o `verify` comando para especificar não verificar atributos.
...
Use o `-nomods` parâmetro com o `verify` comando para especificar não verificar os tempos de modificação do arquivo.
...
Use o `-mtimewindow <s>` parâmetro com o `verify` comando para especificar a diferença de tempo de modificação aceitável para verificação.
...
== verifique -v e verifique -l

Use os `-v` parâmetros e `l` com o `verify` comando para recuperar formatos de saída e listar as diferenças encontradas.
...
== verifique -nomes diferentes

Use o `-nonames` parâmetro com o `verify` comando para excluir nomes de usuários e grupos de listas de arquivos ou relatórios
...
Use o `-match <filter>` parâmetro com o `verify` comando para processar apenas os arquivos e diretórios que correspondem ao filtro.
...
Use o `-bs <n>` parâmetro com o `verify` comando para especificar o tamanho do bloco de leitura/gravação. O valor padrão é 64k.
...
Use o `-parallel <n>` parâmetro com o `verify` comando para especificar o número máximo de processos em lote simultâneos.
...
Use o `-dircount <n[k]>` parâmetro com o `verify` comando para especificar o tamanho da solicitação ao ler diretórios. O valor padrão é 64k.
...
Use o `-noId` parâmetro com o `verify` comando para desativar a criação de um índice padrão. O valor padrão é false.
...
Use o `-preserve-atime` parâmetro com o `verify` comando para restaurar todos os arquivos para a última data acessada na origem. O `-preserve-atime` parâmetro repõe o tempo de acesso ao valor original definido antes da operação de leitura do XCP.
...
== verifique o -s3.insecure

Use o `-s3.insecure` parâmetro com o `verify` comando para usar HTTP em vez de HTTPS para comunicação de bucket S3.
...
Use o `-s3.endpoint <s3_endpoint_url>` parâmetro com o `verify` comando para substituir o URL de endpoint padrão da AWS com um URL especificado para a comunicação de bucket do S3.
...
== verifique o <name> -s3.profile

Use o `s3.profile` parâmetro com o `verify` comando para especificar um perfil do arquivo de credenciais da AWS para comunicação de bucket do S3.
...
== verifique o -s3.noverify

Use o `-s3.noverify` parâmetro com o `verify` comando para substituir a verificação padrão da certificação SSL para comunicação de bucket S3.
